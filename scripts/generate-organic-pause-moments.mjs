// Generate `src/demo/content/dhyh/ads/organic-pause.json` from the
// Tier 3 source-time scene metadata. Run once whenever the Tier 3
// data changes:
//
...
// (`ads/cta-pause.json` — campaign + theme + scenes + objects +
...
//       `ads/cta-pause.json` so both modes share campaign assets.
//
// Why a script (not runtime computation)
// ──────────────────────────────────────
// The output is bundled at build time alongside `ads/cta-pause.json`,
...
const PAUSE_MOMENTS_PATH = join(ROOT, 'src/demo/content/dhyh/ads/cta-pause.json')
...
const output = {
  video_id: 'DHYH',
  _note:
    'Auto-generated by `scripts/generate-organic-pause-moments.mjs` from `tier3.json`. Do not edit by hand — re-run the script when Tier 3 data changes. Theme block is reused verbatim from `ads/cta-pause.json` so Organic Pause uses the same campaign artwork as CTA Pause; per-product descriptions are placeholders because Tier 3 does not carry product copy.',
  campaign: [
    {
