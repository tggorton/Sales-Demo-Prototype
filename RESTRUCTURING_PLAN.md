...
| **6. Hook decomposition** | Split `useDemoPlayback` into 4–7 narrower hooks (scroll engine stays unified). The HANDOFF §9 panel-scroll discontinuity heuristic gets extracted into its own testable slice as part of this split — Phase 7 deliberately doesn't try to test it in-place because the current tightly-coupled shape requires either a full hook-render harness or extraction-first anyway. | **High** | ⏳ Blocked on Phase 7 |
...
`0 → 1 → 2 → 3 → 4 → 5a → 7a → 7b → 6 → 8 → 9` — Phase 5a lands before the test net so the tests pin the new content structure, not the old. Phase 7 splits into 7a (Vitest, pure-function tests — already landed) and 7b (Playwright + hook tests). Phase 5b stays deferred until a real 2nd content tile is on the roadmap. Phase 6 (hook decomposition) waits for 7b so the riskiest refactor has a full test net under it. Phase 9 is post-structural fine-tuning — best done after the test net so any sync regressions get caught.
