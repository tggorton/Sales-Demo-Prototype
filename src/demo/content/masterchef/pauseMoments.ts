// MasterChef pause-moments adapter.
//
// Owns only the MasterChef-specific JSON documents and the resolver
// exports. Shared types + helpers come from `_shared/pauseMoments.ts`
// — zero references to any other content's folder.
//
// Both pause JSONs are auto-generated:
//   - cta-pause.json     ←  scripts/generate-cta-pause-moments.mjs --content masterchef --windows 30-75,135-195
//   - organic-pause.json ←  scripts/generate-organic-pause-moments.mjs --content masterchef
//
...
import { MASTERCHEF_CONTENT_ID } from './timeline'
import ctaPauseMomentsJson from './ads/cta-pause.json'
import organicPauseMomentsJson from './ads/organic-pause.json'

// Re-export shared types + builder so any future MC-specific consumers
...
export function getActivePauseOverlayPayload(
  ...
  return buildPauseOverlayPayload(match.scene, match.campaign, MASTERCHEF_CONTENT_ID)
}
